...
func LoadConfig() (*AppConfig, error) { // Change to return standard error
	...
	if appConfig.File != "" {
		...
		if err != nil {
			return nil, cerr.NewInternalError("call_audit.main.load_config", fmt.Sprintf("could not load config file: %s", err.Error()))
		}
		err = json.Unmarshal(configData, &appConfig)
		if err != nil {
			return nil, cerr.NewInternalError("call_audit.main.parse_config", fmt.Sprintf("could not parse config file: %s", err.Error()))
		}
	}

	// Check if any required field is missing
	if appConfig.Database.Url == "" {
		return nil, cerr.NewInternalError("call_audit.main.missing_data_source", "Data source is required")
	}
	if appConfig.Consul.Id == "" {
		return nil, cerr.NewInternalError("call_audit.main.missing_id", "Service id is required")
	}
	if appConfig.Consul.Address == "" {
		return nil, cerr.NewInternalError("call_audit.main.missing_consul", "Consul address is required")
	}
